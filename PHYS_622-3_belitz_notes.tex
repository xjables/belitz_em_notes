...
uniquely determine the form $f$.\footnote{The proof of this is analogous to that of Proposition \vref{prop:1The-coordinates-completely}. }
...
are defined by $n$-linear forms,\footnote{As per \ref{sub:ch1sec1_2Tensor-spaces}}
...
rank 1.} The tensors of Example \vref{example:1_3_kronecker} are then called
...
\end{cor}
\begin{proof}
$g$ can be considered a real symmetric matrix; by Lemma (\ref{lem:symmetric_implies_diagonalizable}),
...
\begin{proof}
From Corollary (\ref{cor:diag_metric}), we can write ...\end{proof}
\begin{defn}
\textbf{\emph{Normed coordinate systems.}} Basis sets in which the
metric has the form of Theorem (\ref{thm:diag_metric_2}) are called
\emph{normed coordinate systems}. The number $m$ is characteristic
of the space; this is sometimes called \emph{Sylvester's Rigidity
Theorem}.\end{defn}
\begin{example}
...
\end{prop}
\begin{proof}
From Definition \vref{def:2_4_normal_coord_trafo}, $\det g=\det\left(D^{T}gD\right)=\det g$$\left(\det D\right)^{2}$
$\implies\det D=\pm1$. 
\end{proof}
...
Example \vref{example:def_levicivita}, the Levi-Civita is independent
of $\boldsymbol{x}$; that is,\footnote{This is the only justification I could come up with for this equation.
...
\end{eqnarray*}
 As per Definition \vref{definition:3_1_tensor_field}, the Levi-Civita
tensor constitutes a pseudo-tensor field:
\[
...
\end{prop}
\begin{proof}
Let $D$ be a coordinate transformation. By Proposition \vref{prop:3_2_del_trafo}
and Corollary \vref{cor:2_2_covar_coord_trafo}, 
\begin{eqnarray*}
...
We can generalize the concepts of the tensor product defined in Chapter
1 and the trace of a matrix.
\begin{defn}
...
\begin{proof}
See Definition \vref{def:3_4-is-called} and Lemma \vref{lem:3_4_Antisymmetric-Euclidean-2-tensor}.\end{proof}
\begin{example}
...
\end{minipage}}
\begin{rem}
We adopt the conventions outlined in \chapref{Mathematical-preliminaries},
\subref{ch1sec3_4Minkowski-tensors}.
\end{rem}
\noindent %
\doublebox{\begin{minipage}[t]{1\columnwidth}%
...
\begin{rem}
By \chapref{Mathematical-preliminaries}, \subref{ch1sec3_4Minkowski-tensors},
...
\hphantom{}
\begin{rem}
As per Example \vref{example:action_scalar}, $F^{\mu\nu}F_{\mu\nu}$
is a (Minkowski) scalar; therefore the theory is invariant under Lorentz
...
by Proposition \vref{prop:dual_tensor_gradient}:
\begin{equation}
...
~
\begin{rem}
In terms of $\rho$ and $\boldsymbol{j}$, Proposition \vref{prop:The-4-current-obeys}
...
\begin{rem}
By \chapref{Mathematical-preliminaries}, \ref{sub:ch1sec3_4Minkowski-tensors},
...
\end{prop}
\begin{proof}
From Definition \vref{def:energy-momentum-tensor-field},\footnote{In the first line, we use the notational convention that $\partial_{\nu}F{}_{\mu}{}^{\alpha}F{}^{\nu}{}_{\alpha}$
...
by the arguments from \ref{sub:ch2sec2_1Continuity-equation-for}.
...
Since $F^{\mu\nu}$ is an antisymmetric Minkowski tensor, from Chapter
1, \ref{sub:ch1sec3_4Minkowski-tensors} we can write it in the form\begin{eqnarray*} F_{\mu\nu} & = & 
\left(\begin{array}{c;{2pt/2pt}ccc}
...
\end{minipage}
\end{rem}
\ From Chapter 1, \ref{sub:ch1sec3_4Minkowski-tensors}, we can write
$A^{\mu}$ in the form
\[
...
density (see \ref{sub:ch2sec2_1Continuity-equation-for}).
\end{rem}

\subsection{Maxwell's equations}

From \ref{sub:ch2sec1_3The-field-equations} Equation (\ref{eq:dual_zero}),
we have 
\[
...
Now consider, from \ref{sub:ch2sec1_3The-field-equations}, the Euler-Lagrange
...
\end{claim}
\begin{proof}
From \ref{sub:ch2sec3_1The-field-tensor},
\[
...
\partial_{\nu}T^{\mu\nu}=-\frac{1}{c}F^{\mu\nu}J_{\nu}
\]
 from \ref{sub:ch2sec2_3The-continuity-equation} for $\mu=0$:\footnote{Recall that $T^{\mu\nu}:=-\frac{1}{4\pi}F^{\mu\alpha}F{}^{\nu}{}_{\alpha}+\frac{1}{16\pi}g^{\mu\nu}F_{\alpha\beta}F^{\alpha\beta}$
from \ref{sub:ch2sec2_2The-energy-momentum-tensor}, and $F_{\alpha\beta}F^{\alpha\beta}=2\left(\boldsymbol{E}^{2}-\boldsymbol{B}^{2}\right)$
from Chapter 1 \ref{sub:ch1sec3_4Minkowski-tensors}. }
\[
...
conservation. It is analogous to \ref{sub:ch2sec2_1Continuity-equation-for}
with $\rho\rightarrow u$, $\boldsymbol{j}\rightarrow\boldsymbol{P}$.
\end{rem}
~
\begin{rem}
Recall from \ref{sub:ch2sec3_5Charges-in-electromagnetic} that since
...
density due to just one particle, as in \ref{sub:ch2sec3_5Charges-in-electromagnetic}
...
\]
 where the last step follows from \ref{sub:ch2sec3_5Charges-in-electromagnetic}.
...
~
\begin{rem}
The remaining components of the continuity equation from \ref{sub:ch2sec2_3The-continuity-equation},
\[
\partial_{\nu}T^{j\nu}=-\frac{1}{c}F^{j\nu}J_{\nu}
\]
...
\]
 Now let $D{}^{\mu}{}_{\nu}$ be a Lorentz boost. Then, from \ref{sub:ch2sec3_1The-field-tensor}
we have
\begin{eqnarray*}
...
\end{cor}
\begin{proof}
Generalize Proposition \ref{prop:LinearSolutions} to $\alpha=1,\dots,N$
and let $N\rightarrow\infty$.\end{proof}
\begin{rem}
...
By Proposition \ref{prop:LinearSolutions},
\begin{eqnarray*}
...
~
\begin{rem}
From Chapter \ref{chap:Maxwell's-Equations}, \ref{sub:ch2sec3_4Relations-between-fields},
...
solution of Laplace's equation (see Chapter \ref{chap:Maxwell's-Equations}
\ref{sub:ch2sec5_1Real-solutions} Corollary \ref{cor:The-most-general}). 
\end{rem}
~
\begin{rem}
\label{rem:Ch3sec1_2rem5_From-=0000A7-Remark}From \ref{sub:ch3sec1_1Electrostatics}
Remark \ref{rem:A-harmonic-functionextrema}, the only solution of
Laplace's equation that vanishes at infinity is the zero solution;
in an infinite system, there is only one physical solution of Poisson's
equation.
\end{rem}
~
\begin{rem}
...
\begin{proof}
Consider Lemma \ref{lem:inverse_fourier} with $f_{1}=f$, $f_{2}\left(\boldsymbol{y}\right)=e^{-\alpha\boldsymbol{y}^{2}}e^{i\boldsymbol{y}\cdot\boldsymbol{x}}$,
where $\alpha>0$. 
\begin{eqnarray*}
...
\begin{rem}
The inverse Fourier transform is given by the theorem in \ref{sub:ch3sec2_2Inverse-Fourier-transforms}.\end{rem}
\begin{prop}
...
This function fulfills the premise of Proposition \ref{prop:ordinary_generalized_functions}
...
But from \ref{sub:ch3sec2_4Generalized-functions} Example \ref{example:constant_gen_function},
...
\end{cor}
\begin{proof}
From the theorem from \ref{sub:ch3sec2_2Inverse-Fourier-transforms},
\[
...
\begin{prop}
Let $\phi\left(x\right)$ be a weakly increasing function.\footnote{As per \ref{sub:ch3sec2_3Test-functions} Definition \ref{def:Weakly-increasing-functions.}.}
Then
\[
...
\end{prop}
\begin{proof}
From \ref{sub:ch3sec1_1Electrostatics},\footnote{Here and elsewhere, the symbol $\overset{\mathcal{F}}{\longrightarrow}$
...
~
\begin{rem}
\label{rem:ch3sec3_1_rem2=0000A7-Remark-}\ref{sub:ch3sec1_2Magnetostatics}
Remark \ref{rem:Ch3sec1_2rem5_From-=0000A7-Remark} follows immediately:
\begin{eqnarray*}
...
~
\begin{rem}
All of this is consistent with \ref{sub:ch3sec1_2Magnetostatics}
Remark \ref{rem:Ch3sec1_2rem4_linear}.
\end{rem}
...
\begin{proof}
From \ref{sub:ch3sec3_1The-general-solution},
\[
...
\end{eqnarray*}
 From the convolution theorem from \ref{sub:ch3sec2_1The-Fourier-transform},
\begin{eqnarray*}
...
\ref{sub:ch3sec3_2The-Coulomb-potential},\footnote{The primes are not derivatives!}
\[
...
Then $CS$ and $CS'$ are related by a Lorentz boost; from Chapter
\ref{chap:Maxwell's-Equations} \ref{sub:ch2sec4_1Physical-interpretation-of},\footnote{Recall that $\gamma:=\frac{1}{\sqrt{1-\frac{v^{2}}{c^{2}}}}$.}
\begin{eqnarray*}
...
\]
 We boost these, using the results from Chapter \ref{chap:Maxwell's-Equations}
\ref{sub:ch2sec4_2Transformations-of-}:
\begin{eqnarray*}
...
What about $\boldsymbol{B}$? Again, from Chapter \ref{chap:Maxwell's-Equations}
\ref{sub:ch2sec4_2Transformations-of-},
\begin{eqnarray*}
B_{x} & = & B'_{x}=0\\
...
\end{prop}
\begin{proof}
From Chapter \ref{chap:Maxwell's-Equations} \ref{sub:ch2sec3_6Poynting's-theorem},
\begin{eqnarray*}
...
\end{eqnarray*}
where the last line follows from Poisson's formula (\ref{sub:ch3sec3_3Poisson's-formula})\end{proof}
\begin{rem}
...
\end{prop}
\begin{proof}
From \ref{sub:ch3sec1_2Magnetostatics}, each component of $\boldsymbol{A}$
obeys Poisson's equation. The solution for each component is given
by Poisson's formula. \end{proof}
\begin{rem}
The proof in \ref{sub:ch3sec1_2Magnetostatics} Proposition \ref{prop:PoissonsEquationsj}
...
~
\begin{proof}
\emph{(Of Proposition \ref{prop:magnetostatic_interaction})}
...
\item From \ref{sub:ch3sec3_6The-law-of} Proposition \ref{prop:A-stationary-current}.
\end{enumerate}
\end{proof}
\begin{rem}
...
formula (\ref{sub:ch3sec3_3Poisson's-formula}) gives
\begin{eqnarray*}
...
(see \ref{sub:ch3sec3_2The-Coulomb-potential}),
\begin{eqnarray*}
...
\]
 from \ref{sub:ch3sec4_2Legendre-functions-and} Remark \ref{rem:9_spherical_harmonic_propertiesThe--have}.
\[
...
\begin{proof}
\emph{(Of Theorem \ref{thm:Spherical-differential-equation})}
...
\end{eqnarray*}
 (see \ref{sub:ch3sec4_2Legendre-functions-and}). By \ref{sub:ch3sec4_2Legendre-functions-and}
Theorem \ref{thm:Completeness-of-spherical}, any reasonably well
behaved $a\left(r,\Omega\right)$ can be expanded in spherical harmonics:
\[
...
in \ref{sub:ch3sec4_3Separation-of-the} with $V\left(r\right)=0$,
...
is the constant $l=0$ contribution (see \ref{sub:ch3sec1_2Magnetostatics}
Remark \ref{rem:Ch3sec1_2rem5_From-=0000A7-Remark}, \ref{sub:ch3sec3_1The-general-solution}
Remark \ref{rem:ch3sec3_1_rem2=0000A7-Remark-}).
\end{rem}
...
\item From Theorem \ref{thm:Completeness-of-theLegendre} in \ref{sub:ch3sec4_2Legendre-functions-and},
we can expand the square root in terms of Legendre polynomials since
$\cos\gamma\in\left[-1,1\right]$.
\item From the \emph{Addition Theorem} in \ref{sub:ch3sec4_2Legendre-functions-and}.
\end{enumerate}
\textbf{Remaining question:} What is $f_{l}\left(\frac{r_{-}}{r_{+}}\right)$? 

From \ref{sub:ch3sec4_4Expansion-of-harmonic}, $\frac{1}{\left|\boldsymbol{x}-\boldsymbol{x}'\right|}$
...
for $r\rightarrow\infty$. By \ref{sub:ch3sec4_4Expansion-of-harmonic},
...
\begin{proof}
From \ref{sub:ch3sec3_3Poisson's-formula} and inserting the lemma,\footnote{Note that $\left|\boldsymbol{x}\right|:=r$, $\left|\boldsymbol{y}\right|:=y$.}
\begin{eqnarray*}
...
From \ref{sub:ch3sec3_5Electrostatic-interaction}, 
\begin{eqnarray*}
...
\]
 From \ref{sub:ch3sec1_1Electrostatics}, $\phi_{>}\left(\boldsymbol{x}\right)$
...
\end{tabular}
\begin{rem}
Alternatively, we can use the spherical harmonic expansion from \ref{sub:ch3sec4_4Expansion-of-harmonic}
...
\]
 where, as per \ref{sub:ch3sec4_5Multipole-expansion-of}, $Q_{lm}$
...
From \ref{sub:ch3sec3_6The-law-of}, the Biot \& Savart law gives
...
as per \ref{sub:ch3sec4_1The-electric-dipole}.
\item The monopole contribution is zero by Remark \ref{rem:remark1_stationary_m4}:
\[
...
\end{cor}
\begin{proof}
Analogous to \ref{sub:ch3sec4_1The-electric-dipole}.\end{proof}
\begin{prop}
...
in Chapter \ref{chap:Static-solutions-of} \ref{sub:ch3sec1_1Electrostatics},
\ref{sub:ch3sec1_2Magnetostatics}, we saw that in vacuum, static
...
\end{thm}
\begin{proof}
From Chapter \ref{chap:Maxwell's-Equations} \ref{sub:ch2sec1_3The-field-equations},
\begin{eqnarray*}
...
$\boldsymbol{B}$ or $A^{\mu}$. From \ref{sub:ch4sec1_1The-wave-equation}
...
~
\begin{rem}
By the superposition principle from Chapter \ref{chap:Maxwell's-Equations}
...
$\hat{\boldsymbol{n}}$ from \ref{sub:ch4sec1_3Orientation-of-the}
is
\[
\hat{\boldsymbol{n}}=\hat{\boldsymbol{k}}:=\frac{\boldsymbol{k}}{\left|\boldsymbol{k}\right|}=\frac{\boldsymbol{k}}{\nicefrac{\omega}{c}}.
\]
 From \ref{sub:ch4sec1_3Orientation-of-the} we also have 
\[
...
From \ref{sub:ch4sec1_1The-wave-equation}, the general wave equation
is 
\[
\boxed{\square f\left(\boldsymbol{x},t\right)=\left(\frac{1}{c^{2}}\partial_{t}^{2}-\nabla^{2}\right)f\left(\boldsymbol{x},t\right)=0}.\tag{*}
\]
 Take a spacial Fourier transform (Chapter \ref{chap:Static-solutions-of}
...
\begin{itemize}
\item static solutions of Maxwell's equations with sources (Chapter \ref{chap:Static-solutions-of})
\item dynamic solutions of Maxwell's equations in vacuum (Chapter \ref{chap:Electromagnetic-waves-in}).
\end{itemize}
Now we discuss
\begin{itemize}
...
Recall in Chapter \ref{chap:Maxwell's-Equations} \ref{sub:ch2sec3_4Relations-between-fields},
...
~
\begin{rem}
From Chapter \ref{chap:Maxwell's-Equations} \ref{sub:ch2sec3_1The-field-tensor},
...
PDEs, which are the equations of motion (or field equations) for $A^{\mu}\left(x\right)$:\footnote{Compare with Chapter \ref{chap:Electromagnetic-waves-in} \ref{sub:ch4sec1_1The-wave-equation}.}
\[
...
From Chapter \ref{chap:Maxwell's-Equations} \ref{sub:ch2sec2_4Gauge-invariance},
...
\begin{prop}
In Lorenz gauge, the field equations for the potentials \textup{\ref{sub:ch5sec1_1Fields-and-potentials}
$\left(*\right)$ becomes
\[
...
\begin{proof}
Lorenz gauge $\implies\partial_{\mu}A^{\mu}=0,$ $\therefore$ \ref{sub:ch5sec1_1Fields-and-potentials}
...
\begin{rem}
From Chapter \ref{chap:Maxwell's-Equations} \ref{sub:ch2sec2_1Continuity-equation-for},
...
\end{prop}
\begin{proof}
\ref{sub:ch5sec1_1Fields-and-potentials} $\left(*'\right)$ with
...
of \ref{sub:ch5sec2_1The-concept-of} $\left(**\right)$ with respect
...
falls off at $\pm\infty$, or, using Chapter \ref{chap:Static-solutions-of}
\ref{sub:ch3sec2_1The-Fourier-transform} Proposition \ref{prop:fourier_of_derivativeLet--be}.
\end{enumerate}
...
\begin{thm}
The defining equation for the Green's functions (\textup{\ref{sub:ch5sec2_1The-concept-of}
...
\]
 From the proposition in \ref{sub:ch5sec2_1The-concept-of}, the two
solutions of the wave equation with this source are
\begin{eqnarray*}
...
We can obtain the potentials by applying the proposition from \ref{sub:ch5sec2_1The-concept-of}
and results from \ref{sub:ch5sec2_2Green's-functions-for} to the
...
in the static case (cf. Chapter \ref{chap:Static-solutions-of} \ref{sub:ch3sec3_6The-law-of}).
...
Consider retarded potentials (\ref{sub:ch5sec2_3The-retarded-potentials}
...
\end{lem}
\begin{proof}
By the continuity equation (Chapter \ref{chap:Maxwell's-Equations}
\ref{sub:ch2sec2_1Continuity-equation-for}),
\[
...
From \ref{sub:ch5sec1_1Fields-and-potentials}, $\boldsymbol{B}=\nabla\times\boldsymbol{A}$,
...
\end{enumerate}
From \ref{sub:ch5sec1_1Fields-and-potentials}, $\boldsymbol{E}=-\nabla\phi-\frac{1}{c}\partial_{t}\boldsymbol{A}$,
...
(from \ref{sub:ch5sec2_3The-retarded-potentials})
...
From \ref{sub:ch5sec3_2The-radiated-power}, 
\begin{eqnarray*}
...
From \ref{sub:ch5sec3_2The-radiated-power}, to find $\frac{d\mathscr{P}}{d\Omega}$
we need
\begin{eqnarray*}
...
\item By definition (Ch \ref{chap:Static-solutions-of} \ref{sub:ch3sec4_7The-magnetic-moment}),
...
\]
 Define a temporal Fourier transform (cf. \ref{sub:ch5sec2_2Green's-functions-for})\footnote{In this notation, the argument of the function indicates if it is
...
the expansion from \ref{sub:ch5sec3_1Asymptotic-potentials-and} applies:
\[
...
\begin{rem}
This is consistent with Ch. \ref{chap:Electromagnetic-waves-in} \ref{sub:ch4sec1_5Polarization-of-electromagnetic}
remark \ref{rem:ch4sec1_5rem1The-direction-of}.
\end{rem}
~
\begin{rem}
...
follows instantly from the proposition in \ref{sub:ch5sec3_1Asymptotic-potentials-and}
...
\ref{chap:Maxwell's-Equations} \ref{sub:ch2sec3_6Poynting's-theorem}):
\[
...
\]
 Then the total energy $U$ radiated into a solid angle is (see \ref{sub:ch5sec3_2The-radiated-power}):
\begin{eqnarray*}
...
Also, by \ref{sub:ch5sec4_2Asymptotic-potentials-and}, $\boldsymbol{E}\left(\boldsymbol{x},\omega\right)\approx-\hat{\boldsymbol{x}}\times\boldsymbol{B}\left(\boldsymbol{x},\omega\right)$.
\item Since $\hat{\boldsymbol{x}}\perp\boldsymbol{B}$.
\item Since integrand is even in $\omega$.
\item From \ref{sub:ch5sec4_2Asymptotic-potentials-and} proposition, $\boldsymbol{B}\left(\boldsymbol{x},\omega\right)\approx i\frac{\omega}{c}\frac{e^{i\omega r/c}}{r}\hat{\boldsymbol{x}}\times\frac{1}{c}\boldsymbol{j}\left(\boldsymbol{k},\omega\right)$.
\end{enumerate}
\[
...
From \ref{sub:ch5sec4_3The-spectral-distribution}, $\frac{d^{2}U}{\infinitesimal{\Omega}d\omega}$
...
\item By \ref{sub:ch5sec3_4Dipole-radiation} lemma, $\frac{d}{dt}\boldsymbol{d}\left(t\right)=\indefint{\boldsymbol{y}}\boldsymbol{j}\left(\boldsymbol{y},t\right)$.\end{enumerate}
\begin{prop}
...
\begin{rem}
This is consistent with the Larmor formula from \ref{sub:ch5sec3_3Radiation-by-an}
(see Problem \#52).
\end{rem}
~
\begin{example}
...
From \ref{sub:ch5sec4_3The-spectral-distribution}, the spectral distribution
of radiation from a time-dependent current density:
\begin{eqnarray*}
...
Consider a point particle as in \ref{sub:ch5sec3_3Radiation-by-an}:
\[
...
$\implies$ no radiation, in agreement with \ref{sub:ch5sec3_3Radiation-by-an}
...
\end{description}
\begin{lem}
The power spectrum from \textup{\ref{sub:ch5sec5_1The-time-Wigner-function,}
...
\begin{proof}
From \ref{sub:ch5sec5_1The-time-Wigner-function,}, the integrand
...
\begin{enumerate}
\item By definition (see \ref{sub:ch5sec5_1The-time-Wigner-function,}).
\item From \ref{sub:ch5sec5_2Cherenkov-radiation} remark \ref{rem:ch5sec5_2The-Wigner-function}. 
...
Let $D$ be a coordinate transformation. By Claim \vref{claim:3_2_trafo_partial},
\[
